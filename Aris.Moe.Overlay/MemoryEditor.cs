...
namespace Aris.Moe.Overlay
{
    public class MemoryEditor
    {
        ...
        public unsafe void Draw(string title, byte[] mem_data, int mem_size, int base_display_addr = 0)
        {
            ...
            var line_total_count = (mem_size + Rows - 1) / Rows;
            ...
            if ((DataEditingAddr / Rows) != (data_editing_addr_backup / Rows))
            {
                ...
                var scroll_desired = (scroll_offset < 0.0f && DataEditingAddr < visible_start_addr + Rows * 2) ||
            ...
            for (var line_i = clipper.DisplayStart; line_i < clipper.DisplayEnd; line_i++) // display only visible items
            {
                ...
                for (var n = 0; n < Rows && addr < mem_size; n++, addr++)
                {
                    ...
                    if (DataEditingAddr == addr)
                    {
                        ...
                        var flags = ImGuiInputTextFlags.CharsHexadecimal | ImGuiInputTextFlags.EnterReturnsTrue | ImGuiInputTextFlags.AutoSelectAll |
                                    ImGuiInputTextFlags.NoHorizontalScroll | ImGuiInputTextFlags.AlwaysInsertMode | ImGuiInputTextFlags.CallbackAlways;
